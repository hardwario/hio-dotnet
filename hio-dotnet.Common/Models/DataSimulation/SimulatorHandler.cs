...
namespace hio_dotnet.Common.Models.DataSimulation
{
    public class SimulatorHandler
    {
            ...
            if (Simulators.TryGetValue(id, out var sim))
            {
                ...
                if (SimulatorTasks.TryGetValue(id, out var task))
                {
                    try
                    {
                        ...
                        await task; // Počkáme, dokud task simulátoru neskončí
                        Console.WriteLine($"[INFO] Simulator {id} task completed.");
                    }
        ...
        /// Stop All simulators
        /// </summary>
        public async Task StopAllSimulators()
        {
            foreach (var id in Simulators.Keys)
            {
                await StopSimulator(id);
            }
        }
        ...
        {
            return Task.WhenAll(SimulatorTasks.Values); // Čeká na dokončení všech simulátorů
        }
    }
}
