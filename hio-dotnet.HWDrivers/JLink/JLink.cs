...
namespace hio_dotnet.HWDrivers.JLink
{
    ...
    public unsafe struct INTERNAL_JLINKARM_EMU_CONNECT_INFO
    {  // C# equivalent of C definition for JLINKARM_EMU_CONNECT_INFO, only used inside this module
    ...
    public struct JLINKARM_EMU_CONNECT_INFO
    {  // Copy of INTERNAL_JLINKARM_EMU_CONNECT_INFO, except that it does not use any fixed-size arrays for convenience
        public U32 SerialNumber;
        public U32 Connection;
        public U32 USBAddr;
        public U8[] aIPAddr;
    ...
    public struct JLINKARM_DEVICE_INFO
    {    // Copy of INTENRAL_JLINKARM_DEVICE_INFO, except that it does not use any fixed-size arrays for convenience
        public string sName;
        public U32 CoreId;
        public U32 FlashAddr;
        public U32 RAMAddr;
    ...
    [StructLayout(LayoutKind.Sequential)]
    public struct INTERNAL_JTAG_ID_DATA
    {  // C# equivalent of C definition for JTAG_ID_DATA, only used inside this module
    ...
    public struct JTAG_ID_DATA
    {  // Copy of INTERNAL_JTAG_ID_DATA, except that it does not use any fixed-size arrays for convenience
        public int NumDevices;
        public U16 ScanLen;
        public U32[] aId;
        public U8[] aScanLen;
    ...
    public class JLink
    {
        public const int JLINKARM_TIF_SWD = 1;

        public const int JLINKARM_RTTERMINAL_CMD_START = 0;
        public const int JLINKARM_RTTERMINAL_CMD_STOP = 1;
        public const int JLINKARM_RTTERMINAL_CMD_GETDESC = 2;
        public const int JLINKARM_RTTERMINAL_CMD_GETNUMBUF = 3;
        public const int JLINKARM_RTTERMINAL_CMD_GETSTAT = 4;

        public const int JLINKARM_RTTERMINAL_BUFFER_DIR_UP = 0;
        public const int JLINKARM_RTTERMINAL_BUFFER_DIR_DOWN = 1;
    ...
    private const string sDLL = "libjlinkarm";
        ...
        // Přidání JLINK_RTTERMINAL_Read
        [DllImport(sDLL, EntryPoint = "JLINK_RTTERMINAL_Read", CallingConvention = CallingConvention.StdCall)]
        unsafe private static extern int JLINK_RTTERMINAL_Read(int bufferIndex, byte[] pData, int size);

        [DllImport(sDLL, EntryPoint = "JLINK_RTTERMINAL_Write", CallingConvention = CallingConvention.StdCall)]
        private static extern int JLINK_RTTERMINAL_Write(int bufferIndex, byte[] pData, int size);

        [DllImport(sDLL, EntryPoint = "JLINK_RTTERMINAL_Control", CallingConvention = CallingConvention.StdCall)]
        private static extern int JLINK_RTTERMINAL_Control(uint command, ref uint direction);


        [DllImport(sDLL, EntryPoint = "JLINK_BeginDownload", CallingConvention = CallingConvention.Cdecl)]
        public static extern int JLINK_BeginDownload(int flags);

        [DllImport(sDLL, EntryPoint = "JLINK_EndDownload", CallingConvention = CallingConvention.Cdecl)]
        public static extern int JLINK_EndDownload();

        [DllImport(sDLL, EntryPoint = "JLINK_WriteMem", CallingConvention = CallingConvention.Cdecl)]
        public static extern int JLINK_WriteMem(uint address, int size, byte[] buffer);

        [DllImport(sDLL, EntryPoint = "JLINK_ReadMem", CallingConvention = CallingConvention.Cdecl)]
        public static extern int JLINK_ReadMem(uint address, int size, byte[] buffer);

        public unsafe static int RTTerminal_Read(int bufferIndex, byte[] pData, int size)
        {
            return JLINK_RTTERMINAL_Read(bufferIndex, pData, size);
        }

        public unsafe static int RTTerminal_Write(int bufferIndex, byte[] pData, int size)
        {
            return JLINK_RTTERMINAL_Write(bufferIndex, pData, size);
        }

        public unsafe static int RTTerminal_Control(uint command, ref uint direction)
        {
            return JLINK_RTTERMINAL_Control(command, ref direction);
        }
